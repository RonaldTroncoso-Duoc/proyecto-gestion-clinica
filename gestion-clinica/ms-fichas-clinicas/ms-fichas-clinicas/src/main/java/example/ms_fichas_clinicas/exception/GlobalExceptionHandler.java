...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(FichaClinicaNotFoundException.class)
    public ResponseEntity<Object> handleFichaNotFound(FichaClinicaNotFoundException ex) {
        log.warn("Ficha clínica no encontrada: {}", ex.getMessage());
        return buildResponse(ex.getMessage(), HttpStatus.NOT_FOUND, "Ficha Clínica No Encontrada");
    }

    @ExceptionHandler(FeignException.NotFound.class)
    public ResponseEntity<Object> handleFeignNotFound(FeignException.NotFound ex) {
        log.warn("Recurso externo no encontrado");
        return buildResponse(
                "Paciente, médico o cita no encontrada en el sistema",
